...
const impl = {
  ...
  getFunctionName: () => process.env.FUNCTION_NAME,
  readTimeout: (directory = __dirname) =>
    readFileAsync(path.join(directory, 'serverless.yml'))
      .then(safeLoad)
      .then((service) => {
          console.log(`service.functions.loadGenerator.timeout: ${service.functions.loadGenerator.timeout}`)
          console.log(`service.provider.timeout: ${service.provider.timeout}`)
