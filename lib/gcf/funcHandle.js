...
const impl = {
  ...
  addMetadataToInput: (input, { functionUrl, functionName, regionName, projectName }) =>
    Object.assign(
      {},
      input,
      {
  ...
  getFunctionUrl: (req) => req.header('Host'),
  ...
  createHandlerHttp: (
    ...
    taskHandler =>
       (request, response, isColdStart) => {
        console.log(`createHandlerHttp Request handler`);
        var functionExecutionStartTime = Date.now();
         ...
         const context = {
            'functionUrl': getFunctionUrl(request),
            'functionName': getFunctionName(),
            'regionName': getRegionName(),
            'projectName': getProjectName()
          }
