...
const impl = {
  ...
  distribute: (timeNow, script, settings, plans) => {
    ...
    return Promise.all(invocations)
      .then((results) => {
        if (script._trace) {
          console.log(`load test from ${impl.getTimeString(script._genesis)} in ${impl.getTimeString(timeNow)} completed @ ${impl.getTimeString(Date.now())}`)
        }
...
module.exports = {
  loadGenerator: func.handle(impl.handle),
}
