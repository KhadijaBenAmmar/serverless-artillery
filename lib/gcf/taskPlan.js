...
const impl = {
  ...
  splitScriptByDurationInSeconds: (script, chunkSize) => {
    ...
    // assign ids to them
    ret.chunk._id = impl.getRandomInt()
    ret.remainder._id = impl.getRandomInt()
  ...
  splitScriptByDurationInSecondsAndSchedule: (timeNow, event, settings) => {
    ...
    // SPLIT
    if (script._trace) {
      console.log(`splitting script by duration from ${impl.getTimeString(script._genesis)} in ${impl.getTimeString(timeNow)} @ ${impl.getTimeString(Date.now())}`)
    }
    ...
    if (script._trace) {
      console.log(`scheduling immediate chunk start from ${impl.getTimeString(script._genesis)} in ${impl.getTimeString(timeNow)} for execution @ ${impl.getTimeString(parts.chunk._start)}`)
    }
    ...
    if (script._trace) {
      console.log(`scheduling future chunk start from ${impl.getTimeString(script._genesis)} in ${impl.getTimeString(timeNow)} for execution @ ${impl.getTimeString(parts.remainder._start)}`)
    }
    return parts
  },
  /**
  ...
  splitScriptByRequestsPerSecondAndSchedule: (timeNow, event, settings) => {
    ...
    if (script._trace) {
      console.log(`splitting immediate chunk by requests per second from ${impl.getTimeString(script._genesis)} in ${impl.getTimeString(timeNow)} @ ${impl.getTimeString(Date.now())}`)
    }
    if (!script._start) {
    ...
    if (script._trace) {
      console.log(`immediate chunk split in to ${plan.length - initialLength} chunks, with ${initialLength} future chunk(s) from ${impl.getTimeString(script._genesis)} in ${impl.getTimeString(timeNow)} @ ${impl.getTimeString(Date.now())}`)
    }
    return plan
  },
  // ######################
  ...
  planPerformance: (timeNow, event, settings) => {
    if (!event._level) event._level = 1
    const plan = []
    let script = event
    // ## Duration ##
    ...
    if (scriptDurationInSeconds > settings.maxChunkDurationInSeconds) {
      ...
      script = parts.chunk
      parts.remainder._level = Number(event._level) + 1
