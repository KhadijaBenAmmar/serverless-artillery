...
const impl = {
  ...
  splitScriptByDurationInSecondsAndSchedule: (timeNow, event, settings) => {
    ...
    // reset the control plane bit on the job so that we don't carry it forward in its children
    delete script._isControlPlaneJob
    ...
    // future remainder is a potential control plane job
    parts.remainder._isControlPlaneJob = true
    if (script._trace) {
