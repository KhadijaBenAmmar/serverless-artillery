...
const impl = {
   ...
   */
  findScriptPath: (cloudPlatform = impl.cloudPlatform.aws, scriptPath) => {
    if (scriptPath) {
      ...
      if (impl.fileExists(localDefaultScript)) {
        ...
        var directoryName
        if (cloudPlatform === impl.cloudPlatform.gcp) {
          directoryName = 'gcf'
        }
        else if (cloudPlatform === impl.cloudPlatform.aws) {
          directoryName = 'lambda'
        }
        return path.join(__dirname, directoryName, constants.DefaultScriptName)
      }
    }
  },
  /**
   ...
   */
  getInput: (cloudPlatform = impl.cloudPlatform.aws, options) => {
    ...
    if (options.si || options.stdIn) {
      ...
      const scriptPath = impl.findScriptPath(cloudPlatform, options.p || options.path)
      if (scriptPath) {
...
`,
   ...
   */
  findServicePath: (cloudPlatform = impl.cloudPlatform.aws) => {
    ...
    if (impl.fileExists(localServerlessPath)) {
      ...
      var pathToServerlessYml
      if (cloudPlatform === impl.cloudPlatform.gcp) {
        pathToServerlessYml = 'gcf'
      }
      else if (cloudPlatform === impl.cloudPlatform.aws) {
        pathToServerlessYml = 'lambda'
      }
      return path.join(__dirname, pathToServerlessYml)
    }
  },
  /**
  ...
  serverlessRunner: (cloudPlatform = impl.cloudPlatform.aws, options) => {
    ...
    return serverless.init()
      .then(() => { // add our intercepter
        var cloudPlatformConstructorName
        if (cloudPlatform === impl.cloudPlatform.gcp) {
          cloudPlatformConstructorName = 'GoogleIndex'
        }
        else if (cloudPlatform === impl.cloudPlatform.aws) {
          cloudPlatformConstructorName = 'AwsInvoke'
        }
        const invoke = serverless.pluginManager.plugins.find(
          plugin => Object.getPrototypeOf(plugin).constructor.name === cloudPlatformConstructorName // eslint-disable-line comma-dangle
        )
        const { log } = invoke
        invoke.log = (response) => {
  ...
  */
  serverlessLoader: (cloudPlatform = impl.cloudPlatform.aws) => {
    const serverless = new Serverless({
      interactive: false,
      servicePath: impl.findServicePath(cloudPlatform),
    })
...
module.exports = {
  cloudPlatform: impl.cloudPlatform,
  /**
   ...
   * deployment.
   */
  deploy: (cloudPlatform = impl.cloudPlatform.aws, options) => {
    console.log(`${os.EOL}\tDeploying the functions to ${cloudPlatform}...${os.EOL}`)

    return impl.serverlessRunner(cloudPlatform, options).then(() => {
   ...
   */
  invoke: (cloudPlatform = impl.cloudPlatform.aws, options) => impl.getInput(cloudPlatform, options)
    ...
    .then((input) => {
      ...
      logIf(`${os.EOL}\tInvoking load-tests Function${os.EOL}`)
      return impl.serverlessRunner(cloudPlatform, options).then((result) => {
   ...
   */
  kill: (cloudPlatform = impl.cloudPlatform.aws, options) => {
    if (cloudPlatform === impl.cloudPlatform.gcp) {
      throw new Error(`${os.EOL}GCP platform does not support kill command ${os.EOL}`)
    }
    ...
    return impl.serverlessLoader(cloudPlatform)
      .then((serverless) => {
   ...
   * completion.
   */
  remove: (cloudPlatform = impl.cloudPlatform.aws, options) => {
    console.log(`${os.EOL}\tRemoving function from platform: ${cloudPlatform} ...${os.EOL}`)
   ...
   */
  script: (cloudPlatform = impl.cloudPlatform.aws, options) => {
    if (cloudPlatform === impl.cloudPlatform.gcp) {
      throw new Error(`${os.EOL}GCP platform does not support script command yet.${os.EOL}`)
    }
   ...
   */
  configure: (cloudPlatform = impl.cloudPlatform.aws, options) => new BbPromise((resolve, reject) => {
    if (cloudPlatform === impl.cloudPlatform.gcp) {
      throw new Error(`${os.EOL}GCP platform does not support configure command yet.${os.EOL}`)
    }
    const conflicts = []
   ...
   */
  monitor: (cloudPlatform = impl.cloudPlatform.aws, options) => {
    if (cloudPlatform === impl.cloudPlatform.gcp) {
      throw new Error(`${os.EOL}GCP platform does not support monitor command.${os.EOL}`)
    }
