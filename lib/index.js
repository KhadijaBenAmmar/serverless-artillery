...
const constants = {
  GcloudCredentialsFileArgument: '--gcp_creds_file',
  ...
  findScriptPath: (scriptPath) => {
    if (scriptPath) {
      ...
      if (impl.fileExists(localDefaultScript)) {
        ...
        return path.join(__dirname, 'gcf', constants.DefaultScriptName)
      }
    }
  },
  /**
...
`,
  ...
  findServicePath: () => {
    ...
    if (impl.fileExists(localServerlessPath)) {
      ...
      return path.join(__dirname, 'gcf')
    }
  },
  /**
    ...
    return serverless.init()
      .then(() => { // add our intercepter
        const invoke = serverless.pluginManager.plugins.find(
          plugin => Object.getPrototypeOf(plugin).constructor.name === 'GoogleIndex' // eslint-disable-line comma-dangle
        )
        const { log } = invoke
        invoke.log = (response) => {
  ...
  */
  ...
  setCredentialFileInEnvVariableBestEffort: (options) => {

    var processArgs = process.argv
    if (processArgs && processArgs.indexOf(constants.GcloudCredentialsFileArgument) >= 0) {
      if (options.verbose) {
        console.log(`${constants.GcloudCredentialsFileArgument} argument already exists, not setting the system derived value`)
      }
      return
    }
    ...
    if (fs.existsSync(defaultGcloudDir)) {
        ...
        if (stat.isDirectory()) {
          ...
          if (options.verbose) {
            console.log(`.gcloud directory does not exist in ${homeDir} directory`)
          }
          return
        }
    }
    ...
    if (jsonFiles.length === 1) {
      ...
      process.argv.push(`${constants.GcloudCredentialsFileArgument}`)
      process.argv.push(jsonCredFile)
      if (options.verbose) {
        console.log(`Setting the environment varilable ${constants.GcloudCredentialsFileArgument} to ${jsonCredFile}`)
      }
    }
    else if (jsonFiles.length > 1) {
      console.log(`Multiple json files in ${homeDir} = ${JSON.stringify(jsonFiles)}, please set the path to the file explicitly by ${constants.GcloudCredentialsFileArgument} argument`)
    }
    else {
      console.log(`No service account credentials json file present in default gcloud directory: ${defaultGcloudDir}. Please have one set there`)
    }
...
module.exports = {
   ...
   */
  deploy: (options) => {
    impl.setCredentialFileInEnvVariableBestEffort(options)


    console.log(`${os.EOL}\tDeploying to Google Cloud Functions...${os.EOL}`)
  ...
  remove: (options) => {
    impl.setCredentialFileInEnvVariableBestEffort(options)
    console.log(`${os.EOL}\tRemoving GCF from GCP...${os.EOL}`)
