...
const impl = {
   ...
   */
  findScriptPath: (cloudProvider = impl.cloudProvider.aws, scriptPath) => {
    if (scriptPath) {
      ...
      if (impl.fileExists(localDefaultScript)) {
        ...
        var directoryName
        if (cloudProvider === impl.cloudProvider.google) {
          directoryName = 'gcf'
        }
        else if (cloudProvider === impl.cloudProvider.aws) {
   ...
   */
  getScriptText: (cloudProvider = impl.cloudProvider.aws, options) => {
    ...
    if (options.si || options.stdIn) {
      ...
      const scriptPath = impl.findScriptPath(cloudProvider, options.p || options.path)
      if (scriptPath) {
...
`,
   ...
   */
  findServicePath: (cloudProvider = impl.cloudProvider.aws) => {
   ...
   */
  serverlessRunner: (cloudProvider = impl.cloudProvider.aws, options) => {
    // pretend that SLS was called.
    process.argv[1] = Serverless.dirname
    // proceed with using SLS
    const serverless = new Serverless({
      interactive: false,
      servicePath: impl.findServicePath(cloudProvider),
    })
    ...
    return serverless.init()
      .then(() => { // add our intercepter
        var cloudProviderConstructorName
        if (cloudProvider === impl.cloudProvider.google) {
          cloudProviderConstructorName = 'GoogleIndex'
        }
        else if (cloudProvider === impl.cloudProvider.aws) {
          cloudProviderConstructorName = 'AwsInvoke'
        }
        const invoke = serverless.pluginManager.plugins.find(
          plugin => Object.getPrototypeOf(plugin).constructor.name === cloudProviderConstructorName // eslint-disable-line comma-dangle
        )
        const { log } = invoke
        invoke.log = (response) => {
  ...
  */
  serverlessLoader: (cloudProvider = impl.cloudProvider.aws) => {
    const serverless = new Serverless({
      interactive: false,
      servicePath: impl.findServicePath(cloudProvider),
    })
...
module.exports = {
  cloudProvider: impl.cloudProvider,
  /**
   ...
   * deployment.
   */
  deploy: (cloudProvider = impl.cloudProvider.aws, options) => impl.serverlessLoader(cloudProvider)
    .then((serverless) => {
      impl.validateServiceForDeployment(serverless.service)
      console.log(`${os.EOL}\tDeploying the functions to ${cloudProvider}...${os.EOL}`)
      return impl.serverlessRunner(cloudProvider, options).then(() => {
   ...
   */
  invoke: (cloudProvider = impl.cloudProvider.aws, options) => impl.getScriptText(cloudProvider, options)
    ...
    .then((input) => {
      ...
      return impl.serverlessRunner(cloudProvider, options).then((result) => {
   ...
   */
  kill: (cloudProvider = impl.cloudProvider.aws, options) => {
    if (cloudProvider === impl.cloudProvider.google) {
    ...
    return impl.serverlessLoader(cloudProvider)
      .then((serverless) => {
   ...
   * completion.
   */
  remove: (cloudProvider = impl.cloudProvider.aws, options) => {
    console.log(`${os.EOL}\tRemoving function from platform: ${cloudProvider} ...${os.EOL}`)
   ...
   */
  script: (cloudProvider = impl.cloudProvider.aws, options) => {
    if (cloudProvider === impl.cloudProvider.google) {
   ...
   */
  configure: (cloudProvider = impl.cloudProvider.aws, options) => new BbPromise((resolve, reject) => {
    if (cloudProvider === impl.cloudProvider.google) {
